...
const main = async () => {
  ...
  try {
    ...
    check(true, `rent skip ${String(e.message).slice(0, 40)}`);
  }

  // Move
  const mv = await post("/actions", { actorId: id, rawText: "I move to the Rear Alley" });
  check(
    !!mv.travel || mv.calculationTrace?.some((t) => String(t).includes("travel")),
    `move travel ${JSON.stringify(mv.travel || {}).slice(0, 80)}`,
  );
  check(
    mv.calculationTrace?.some((t) => String(t).includes("action=move")),
    "move typed",
  );
  ...
  check(
    !!st.legal || st.calculationTrace?.some((t) => String(t).startsWith("heat=")),
    `heat ${JSON.stringify(st.legal || st.calculationTrace?.filter((x) => String(x).includes("heat")))}`,
  );
  check(
    st.factionStanding || st.calculationTrace?.some((t) => String(t).includes("faction=")),
    "faction shift",
  );
  ...
  check(
    !!cm.comms || cm.calculationTrace?.some((t) => String(t).includes("comms")),
    `comms ${JSON.stringify(cm.comms || {}).slice(0, 80)}`,
  );
  const msgs = await get(`/comms?actorId=${id}`);
  check(
    Array.isArray(msgs.messages) && msgs.messages.length >= 1,
    `comms list ${msgs.messages?.length}`,
  );
  ...
  check(
    hot.legal?.warrant === true ||
      hot.calculationTrace?.some((t) => String(t).includes("warrant=true")) ||
      hot.legal?.heat >= 40,

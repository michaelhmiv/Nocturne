...
const main = async () => {
  const c = await post("/characters", { name: "Cash Runner", conceptSummary: "street courier with grit" });
  ...
  try { await post("/residences/starter/rent", { characterId: c.characterId }); } catch {}

  const work = await post("/actions", { actorId: c.characterId, rawText: "I work a courier gig tonight" });
  check(!!work.payday || work.calculationTrace?.some((t) => String(t).startsWith("payday=")), `payday ${JSON.stringify(work.payday || {})}`);

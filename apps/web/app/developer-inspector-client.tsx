...
type InspectorTab =
  "overview" | "state" | "relations" | "events" | "plans" | "traces" | "simulation" | "context";
...
export default function DeveloperInspectorClient() {
  ...
  async function load() {
    setLoading(true);
    try {
      const player = await gameFetch<PlayerDashboard>(
        "persistent-world/dashboard?historyLimit=100",
      );
  ...
  const tabs: Array<{ key: InspectorTab; label: string; count?: number }> = [
    ...
    { key: "events", label: "Events", count: entity.recentEvents.length },
    {
      key: "plans",
      label: "Plans",
      count: entity.activePlans.length + entity.scheduledWork.length,
    },
    ...
    <main className="developer-inspector">
      <header className="developer-inspector__hero">
        <div>
          ...
          <h1>{entity.name}</h1>
          <span>
            {entity.definitionType} · {entity.lifecycleStatus}
          </span>
        </div>
        <div>
        ...
        <div className="developer-inspector__grid">
          <section>
            ...
            <dl>
              ...
              <div>
                <dt>Definition ID</dt>
                <dd>{entity.definitionId}</dd>
              </div>
              <div>
                <dt>World</dt>
                <dd>{entity.worldId}</dd>
              </div>
              <div>
                <dt>Shard</dt>
                <dd>{entity.shardId}</dd>
              </div>
              <div>
                <dt>Location</dt>
                <dd>{entity.locationId || "None"}</dd>
              </div>
              <div>
                <dt>Owner</dt>
                <dd>{entity.ownerId || "None"}</dd>
              </div>
              <div>
                <dt>Controller</dt>
                <dd>{entity.controllerId || "None"}</dd>
              </div>
            </dl>
          </section>
          <section>
            ...
            <div className="developer-metric-grid">
              <article>
                <strong>{operator.traces.length}</strong>
                <span>action requests</span>
              </article>
              <article>
                <strong>{entity.recentEvents.length}</strong>
              ...
              <article>
                <strong>{entity.activePlans.length}</strong>
                <span>active plans</span>
              </article>
              <article>
                <strong>{entity.scheduledWork.length}</strong>
                <span>scheduled work</span>
              </article>
              <article>
                <strong>{entity.simulationRuns.length}</strong>
                ...
                <tbody>
                  {operator.handlers.map((handler) => (
                    <tr key={handler.actionKind}>
                      <td>{handler.actionKind}</td>
                      <td>{handler.handlerVersion}</td>
                      <td>{handler.authorityMode}</td>
                      <td>{handler.supportsStateChange ? "yes" : "no"}</td>
          ...
          <section className="developer-inspector__wide developer-warning">
            ...
            <p>
              Repairs remain operator-only and must create version-checked authoritative operations
              or compensating events. This inspector intentionally does not provide arbitrary JSON
              editing.
            </p>
          </section>
        </div>
        ...
        <div className="developer-inspector__grid">
          <section>
            ...
            <dl>
              <div>
                <dt>Condition</dt>
                <dd>{dashboard.character.condition}</dd>
              </div>
              <div>
                <dt>Cash</dt>
                <dd>{dashboard.character.cashOnPerson}</dd>
              </div>
              <div>
                <dt>Heat</dt>
                <dd>{dashboard.character.heat}</dd>
              </div>
              <div>
                <dt>Status</dt>
                <dd>{dashboard.character.status}</dd>
              </div>
            </dl>
            <h3>Resources</h3>
            {dashboard.character.resources.length ? (
              dashboard.character.resources.map((resource) => (
                <div className="developer-state-row" key={resource.key}>
            ...
            {dashboard.character.activeConditions.length ? (
              dashboard.character.activeConditions.map((condition) => (
                <div className="developer-state-row" key={condition.key}>
                  <span>{condition.name}</span>
                  <strong>{condition.intensity}</strong>
                </div>
              ))
            ) : (
        ...
        <div className="developer-inspector__stack">
          <section>
            ...
            {entity.aliases.length ? (
              <JsonBlock value={entity.aliases} />
            ) : (
              <Empty>No aliases.</Empty>
            )}
          </section>
          <section>
            <h2>Relations</h2>
            {entity.relations.length ? (
              <JsonBlock value={entity.relations} />
            ) : (
      ...
      {tab === "events" && (
        <div className="developer-trace-list">
          {entity.recentEvents.length ? (
            entity.recentEvents.map((raw, index) => {
              ...
              return (
                <article key={id}>
                  <header>
                    <div>
                      <strong>
                        {label(String(value(event, "event_type", "eventType") || "event"))}
                      </strong>
                      <code>{id}</code>
                    </div>
                    <time>{when(value(event, "world_time", "occurredAt", "created_at"))}</time>
                  </header>
      ...
      {tab === "plans" && (
        <div className="developer-inspector__stack">
          <section>
            <h2>Active plans</h2>
            {entity.activePlans.length ? (
              <JsonBlock value={entity.activePlans} />
            ) : (
              <Empty>No active plans.</Empty>
            )}
          </section>
          <section>
            <h2>Scheduled work</h2>
            {entity.scheduledWork.length ? (
              <JsonBlock value={entity.scheduledWork} />
            ) : (
      ...
      {tab === "traces" && (
        <div className="developer-trace-list">
          {operator.traces.length ? (
            operator.traces.map((trace) => (
              <article key={trace.requestId}>
                <header>
                  <div>
                    <strong>{trace.command}</strong>
                    <code>{trace.requestId}</code>
                  </div>
                  <div>
                    <span className={`developer-status developer-status--${trace.status}`}>
                      {label(trace.status)}
                    </span>
                    <time>{when(trace.createdAt)}</time>
                  </div>
                </header>
                <dl className="developer-trace-meta">
                  <div>
                    <dt>Plan</dt>
                    <dd>{trace.planId || "None"}</dd>
                  </div>
                  <div>
                    <dt>Context compilation</dt>
                    <dd>{trace.contextCompilationId || "None"}</dd>
                  </div>
                  <div>
                    <dt>Error</dt>
                    <dd>{trace.errorCode || "None"}</dd>
                  </div>
                  <div>
                    <dt>Updated</dt>
                    <dd>{when(trace.updatedAt)}</dd>
                  </div>
                </dl>
                  ...
                  {trace.stages.map((stage) => (
                    <li key={stage.stageId}>
                      ...
                      <div>
                        ...
                        <small>
                          {label(stage.status)} · {when(stage.startedAt)}
                        </small>
                      </div>
                      <details>
                        <summary>Stage payloads</summary>
                        <JsonBlock
                          value={{ input: stage.inputSummary, output: stage.outputSummary }}
                        />
                      </details>
                    </li>
                  ))}
                ...
                <details>
                  ...
                  <JsonBlock
                    value={{
                      authoritative: trace.authoritativeResult,
                      playerSafe: trace.playerSafeResult,
                    }}
                  />
                </details>
              </article>
      ...
      {tab === "simulation" && (
        <div className="developer-inspector__stack">
          <section>
            ...
            {entity.simulationRuns.length ? (
              <JsonBlock value={entity.simulationRuns} />
            ) : (
              <Empty>No simulation runs.</Empty>
            )}
          </section>
        </div>
      )}
      ...
      {tab === "context" && (
        <div className="developer-inspector__stack">
          <section>
            <h2>Latest context inclusion reasons</h2>
            {entity.latestContextReasons.length ? (
              <JsonBlock value={entity.latestContextReasons} />
            ) : (
              <Empty>No context inclusion records.</Empty>
            )}
          </section>
        </div>
      )}

...
import {
  ...
  ScheduledWorkServiceError,
} from "./scheduled-work-service.js";
...
export async function registerScheduledWorkRoutesFromEnv(
  ...
  if (!databaseUrl)
    throw new Error("DATABASE_URL is required for scheduled-work routes.");
  if (!workerSecret)
    throw new Error(
      "AI_JOB_WORKER_SECRET is required for scheduled-work routes.",
    );
  ...
  app.post(
    "/v1/internal/scheduled-actions/:scheduleId/resolve",
    async (request, reply) => {
      const suppliedSecret = String(
        request.headers["x-nocturne-worker-secret"] || "",
      );
    ...
    `;
      ...
      try {
        ...
        const requestId =
          typeof row.payload?.requestId === "string"

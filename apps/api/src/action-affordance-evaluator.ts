...
export function evaluateActionAffordance(
  ...
  const actor = context.entities.find(({ entityId }) => entityId === frame.actorId);
  if (!actor) {
  ...
  if (actor.condition !== undefined && actor.condition <= 0) {
    return result(frame, context, {
      status: "blocked",
      rationale: "The acting character has no remaining physical condition and cannot perform the action.",
  ...
  if (missingTargets.length > 0) {
    return result(frame, context, {
      ...
      rationale: "At least one referenced target is not established in the relevant authoritative context.",
  ...
  if (unreachableTargets.length > 0) {
    return result(frame, context, {
      status: "blocked",
      rationale: "The requested target is not reachable from the actor's authoritative current location.",
      missingRequirements: unreachableTargets.map(({ name }) => `reach ${name}`),
      warnings: [],
    });
  }
  ...
  if (
    ...
    return result(frame, context, {
      status: "blocked",
      rationale: "The actor is authoritatively restrained and lacks the freedom of movement required.",
  ...
  const missingTools = frame.toolIds.filter((id) => {
    ...
    return !tool || !tool.inclusionReasons.some((reason) => ["owned", "controlled", "possessed"].includes(reason));
  });
  ...
  const warnings = [
    ...
    ...(frame.properties.illegal ? ["The action may create legal, heat, or reputation consequences."] : []),
    ...(frame.demands.danger >= 5 ? ["The action presents substantial physical danger."] : []),
  ];
  return result(frame, context, {
    status: "feasible",
    rationale: "The actor, relevant targets, location, and required controlled tools satisfy current affordance checks.",
    missingRequirements: [],
    warnings,
  });
}

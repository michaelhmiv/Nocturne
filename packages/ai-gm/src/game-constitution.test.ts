...
describe("shared game constitution", () => {
  it("defines the persistent improvisational boundary", () => {
    ...
    expect(prompt).toContain(
      "Do not create durable entities merely because a noun appears",
    );
    expect(prompt).toContain(
      "weapons, ammunition, credentials, keys, vehicles",
    );
    expect(prompt).toContain(
      "completed low-impact or no-effect in-world resolution",
    );
  });

  it("requires every AI task to declare its context boundary", () => {
    expect(Object.keys(AI_TASK_CONTEXT_DECLARATIONS).sort()).toEqual(
      expectedTasks.sort(),
    );

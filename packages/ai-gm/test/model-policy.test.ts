...
import { DEFAULT_GENERATIVE_MODEL, createModelPolicy } from "../src/index.js";

const originalAiModel = process.env.AI_MODEL;
const originalGenerativeModel = process.env.AI_GENERATIVE_MODEL;

afterEach(() => {
  if (originalAiModel === undefined) delete process.env.AI_MODEL;
  else process.env.AI_MODEL = originalAiModel;
  if (originalGenerativeModel === undefined) delete process.env.AI_GENERATIVE_MODEL;
  else process.env.AI_GENERATIVE_MODEL = originalGenerativeModel;
});

describe("createModelPolicy", () => {
  it("uses the production default for authoritative tasks", () => {
    delete process.env.AI_MODEL;
    delete process.env.AI_GENERATIVE_MODEL;
    const policy = createModelPolicy({ task: "parse_intent" });

    expect(policy.model).toBe(DEFAULT_GENERATIVE_MODEL);

...
import { geodesicDistanceMeters, projectDiscoverablePlaces, validWorldPoint } from "./discoverable-place-projection.js";
...
const far = { ...food, provider_feature_id: "node:far-grocery", name: "Far Grocery",
  centroid_longitude: -73.96, centroid_latitude: 40.8 };
...
describe("player-scoped geography projection", () => {
  ...
  it("sorts genuine nearby POIs and excludes far or unclassified entities", () => {
    ...
    expect(projectDiscoverablePlaces(origin, [
      far, { ...food, provider_feature_id: "node:private", properties: {} }, food,
  ...
  it("changes neighborhood evidence when the actor moves", () => {
    expect(projectDiscoverablePlaces({ longitude: -73.96, latitude: 40.8 }, [food, far]))
      .toEqual([expect.objectContaining({ sourceKey: "osm:node:far-grocery" })]);
  });
});
